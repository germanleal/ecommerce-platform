...
import { completeLogin, login, restoreSession } from '../services/auth/oidc';
...
function CartPage() {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      const responses = await Promise.all([...groups.values()].map((lines) => orderApi.createMarketplace({ purchaseId, tenantId: lines[0].tenantId, storeId: lines[0].storeId, currency: lines[0].currency, lines: lines.map((item) => ({ productId: item.productId, quantity: item.quantity })) })));
      setOrders(responses.map((response) => response.data)); clear();
    } catch (cause) { setError(apiMessage(cause)); }
    finally { setSubmitting(false); }
  };
  if (orders.length) return <div className="page"><p className="eyebrow">Purchase created</p><h1>Thank you</h1><p>Purchase {orders[0].purchaseId} contains one fulfillment order per store.</p>{orders.map((order) => <div className="order-confirmation" key={order.id}><span>Store order</span><strong>{order.orderNumber}</strong><span>Status</span><strong>{order.status}</strong><span>Total</span><strong>{Number(order.total).toFixed(2)} {order.currency}</strong></div>)}<a className="button" href="/stores">Continue shopping</a></div>;
