...
@Service public class OrderService {
 ...
 @Transactional public Order create(UUID customerId,UUID storeId,Currency currency,List<ItemCommand> commands){if(customerId==null||storeId==null||currency==null||commands==null||commands.isEmpty())throw new IllegalArgumentException("INVALID_ORDER");UUID tenantId=t();Order order=Order.create(tenantId,new OrderNumber("ORD-"+Instant.now().toEpochMilli()+"-"+UUID.randomUUID().toString().substring(0,8)),customerId,storeId,currency);for(var command:commands){var snap=commerce.resolve(tenantId,command.productId());if(!currency.getCurrencyCode().equals(snap.currency()))throw new IllegalArgumentException("INVALID_MONEY");order.addItem(new OrderItem(null,order.id(),snap.productId(),snap.name()+"|sku="+snap.sku(),command.quantity(),new Money(snap.amount(),currency),null));}order.submit();var saved=orders.save(order);emit("ORDER_CREATED",saved);return saved;}
 @Transactional public Order addLine(UUID id,UUID productId,int quantity){Order o=get(id);var snap=commerce.resolve(t(),productId);if(!o.currency().getCurrencyCode().equals(snap.currency()))throw new IllegalArgumentException("INVALID_MONEY");o.addItem(new OrderItem(null,o.id(),snap.productId(),snap.name()+"|sku="+snap.sku(),quantity,new Money(snap.amount(),o.currency()),null));return orders.save(o);}
