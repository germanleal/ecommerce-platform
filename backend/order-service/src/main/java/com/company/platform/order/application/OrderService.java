...
@Service public class OrderService {
 ...
 @Transactional public Order confirmForTenant(UUID tenantId,UUID id){Order o=getForTenant(tenantId,id);o.confirm();var saved=orders.save(o);emit("ORDER_CONFIRMED",saved);return saved;}
 @Transactional public Order startProcessingForTenant(UUID tenantId,UUID id){Order o=getForTenant(tenantId,id);o.startProcessing();var saved=orders.save(o);emit("ORDER_PROCESSING",saved);return saved;}
 @Transactional public Order completeForTenant(UUID tenantId,UUID id){Order o=getForTenant(tenantId,id);o.complete();var saved=orders.save(o);emit("ORDER_COMPLETED",saved);return saved;}
 @Transactional public Order cancelForTenant(UUID tenantId,UUID id){Order o=getForTenant(tenantId,id);o.cancel();var saved=orders.save(o);emit("ORDER_CANCELLED",saved);return saved;}
 ...
 public Order getForTenant(UUID tenantId,UUID id){return orders.findByTenantIdAndId(tenantId,id).orElseThrow(()->new NoSuchElementException("ORDER_NOT_FOUND"));}
 public List<Order> listForTenant(UUID tenantId,OrderStatus status){return orders.findByTenantId(tenantId,status);}
 ...
 private void emit(String type,Order o){events.publish(new OrderEvent(UUID.randomUUID(),type,o.id(),o.tenantId(),Instant.now(),Map.of("orderId",o.id().toString(),"purchaseId",o.purchaseId().toString(),"tenantId",o.tenantId().toString(),"customerId",o.customerId().toString(),"total",o.total().amount(),"currency",o.currency().getCurrencyCode(),"lines",o.items())));}
